...
- **Schätzung vor dem Lauf:** `generate` gibt `Cost estimate (upfront): …`
...
Rolle („technischer Redakteur“), Zielsprache und Anrede fest (`en-you`, der
Default, oder `formal-sie` / `informal-du` für deutsche Endnutzer-Doku),
verbietet erfundene UI-Elemente und verlangt, Lücken explizit zu
kennzeichnen, statt sie zu füllen.
...
| `ductus graph [--open] [--out <path>] [--journey]` | Mermaid auf stdout; `--open` rendert HTML nach `.ductus/graph.html`; `--journey` gibt die Flow-Hauptpfade als `journey`-Diagramme aus |
| `ductus help [command]` | Gibt eine ausführliche CLI-Übersicht aus — bzw. mit Befehlsnamen die Hilfe zu genau diesem Befehl |

Globale Optionen: `-c, --config <path>` (Default `./ductus.config.yaml`) und
...
Das journey-Template bringt mit: interaktive Journey-Graphen (anklickbare
...
je Schritt (`file:line · symbol`), responsives Layout und
`prefers-reduced-motion`-Unterstützung. Die UI des Templates ist standardmäßig
Englisch und wechselt zu Deutsch, wenn die konfigurierte Locale mit `de`
beginnt. Der LLM-Markdown wird zur Buildzeit XSS-sicher gerendert.
...
zusammen und baut es statisch (`astro build`).

### Diagramme in der generierten Doku

Mit `output.website.diagrams: true` (Default) erhält jede Flow-Seite im MDX-
bzw. Starlight-Modus bis zu zwei Mermaid-Abschnitte: den **Hauptpfad** (ein
lineares `journey`-Diagramm) und das **Ablaufdiagramm** des vollständigen
Segments. Der Hauptpfad wird deterministisch abgeleitet: ab `flow.start`
wählt Ductus pro Schritt genau eine ausgehende Kante — Nicht-`back`-Trigger
vor `back`, Kanten ohne `condition` vor solchen mit, bei Gleichstand die
kleinste `edge.id`. Das journey-Template braucht die Mermaid-Diagramme nicht:
Es rendert den Graphen nativ als interaktive Ansicht direkt aus
`ductus.data.json`.
...
- `app.locale` (Default `en`) ist die Sprache der generierten Doku;
  `style.voice` (Default `en-you`) legt die Anrede fest — `formal-sie` und
  `informal-du` erzeugen deutsche Endnutzer-Doku.
...
- **Jeden Node einem Flow zuordnen, `condition` an jede Decision-Kante.**
  Nodes ohne Flow sammeln sich auf einer Sammelseite ohne
...
- **`ductus check` in CI.** Ohne LLM-Aufrufe, ohne Kosten; es gelten die
  ...
  meldet `check` nur als „not generated yet“ (Exit bleibt 0).
