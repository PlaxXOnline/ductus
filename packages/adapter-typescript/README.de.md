...
`npx ductus help <command>` zeigt die Optionen jedes Befehls. Der relevante
Ausschnitt der `ductus.config.yaml` (so erzeugt sie `ductus init`):
...
| `--config <json-file>` | JSON-Konfigurationsdatei (Schlüssel siehe unten); `@ductus/core` erzeugt sie automatisch aus dem Adapter-Eintrag der `ductus.config.yaml` |
