...
statische Website. `@ductus/core` ist das Herzstück der Toolchain: CLI,
Orchestrator, LLM-Schicht und Ausgabe-Module.
...
```bash
cd my_project                        # Flutter- oder TS/JS-Projekt
...
| Option | Beschreibung |
|---|---|
| `-c, --config <path>` | Pfad zur `ductus.config.yaml` (Default: `./ductus.config.yaml`) |
...
| `ductus graph` | `--open`, `--out <path>`, `--journey` | Gibt den Graphen als Mermaid-Flowchart auf stdout aus. `--journey` gibt stattdessen die journey-Diagramme der Flow-Hauptpfade aus. `--out` schreibt in eine Datei. `--open` schreibt `.ductus/graph.html` (Flowchart **und** Journeys) und öffnet sie im Browser. |
| `ductus help [command]` | — | Gibt ohne Argument eine ausführliche CLI-Übersicht aus (Workflow, Befehle, Exit-Codes, Konfiguration); mit Argument die Hilfe zum jeweiligen Befehl. |
...
`app.locale` (Default: `en`) ist die Sprache der generierten
Endnutzer-Dokumentation. `style.voice` (Default: `en-you`) bestimmt ihren
Ton: `en-you` spricht die Lesenden im schlichten englischen „you“ an;
`formal-sie` und `informal-du` bleiben für deutsche Endnutzer-Doku voll
unterstützt (förmliches „Sie“ bzw. informelles „du“).
...
Weitere optionale Schlüssel (mit Defaults, wo zutreffend):
...
|---|---|
| `journey` *(Default)* | Interaktive Journey-Site aus `ductus.data.json`: klickbarer Journey-Graph mit deterministischem Layout, „Pfad abspielen“, ⌘K/Ctrl+K-Suche über Journeys/Schritte/Aktionen, Schrittliste + ausführliche LLM-Anleitung je Journey. Die Site-UI folgt `app.locale` (standardmäßig Englisch, deutsche UI bei `de`). [Template ansehen](https://github.com/PlaxXOnline/ductus/tree/main/templates/journey) |
...
geschrieben, damit du die beanstandeten Stellen prüfen kannst.
...
den Ordner `.ductus/cache` mit ins Repository einchecken (er stammt aus
deinem letzten lokalen `ductus generate`).
...
steps:
  ...
  - uses: actions/setup-node@v4
    with:
      node-version: 20
  - uses: subosito/flutter-action@v2   # der Dart-Adapter braucht das Dart/Flutter-SDK
