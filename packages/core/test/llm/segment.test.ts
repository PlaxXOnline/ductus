...
describe('segmentGraph — flow-Granularität', () => {
  it('erzeugt je Flow ein Segment (nach id sortiert) plus _misc mit stabiler Reihenfolge', () => {
  ...
  it('nimmt nur Edges mit beiden Enden im Segment auf und sortiert nodes/edges nach id', () => {
  ...
  it('führt segment-verlassende Edges als exits mit toTitle des Ziel-Nodes', () => {
    const segments = segmentGraph(graph, 'flow');
  ...
  it('nimmt den Start-Node auch ohne eigene flow-Zuordnung ins Flow-Segment auf', () => {
    ...
    // Start-Node zählt zum Flow ⇒ kein _misc-Segment.
    expect(segments.map((s) => s.id)).toEqual(['main']);
    expect(segments[0]!.nodes.map((n) => n.id)).toEqual(['about', 'home']);
  });

  it('lässt _misc weg, wenn alle Nodes einem Flow zugeordnet sind', () => {
...
describe('segmentGraph — screen-Granularität', () => {
  it('erzeugt je Screen-Node ein Segment in id-Reihenfolge; Nicht-Screens nur als exits-Ziele', () => {
    ...
    // Decision-Node "gate" hat kein eigenes Segment.
    expect(segments.find((s) => s.id === 'gate')).toBeUndefined();
  });

  it('enthält genau den Screen als Node; ausgehende Edges landen nur in exits (keine Duplikate)', () => {
    ...
    // Regression: edges und exits müssen disjunkt sein (contracts.ts: exits = Kanten,
    // die das Segment verlassen) — sonst doppelte Kanten in Mermaid-Diagramm und Prompt.
  ...
  it('behält Self-Loops in edges und führt sie nicht als exits', () => {
  ...
  it('liefert Segmente ohne ausgehende Edges mit leeren edges/exits', () => {
